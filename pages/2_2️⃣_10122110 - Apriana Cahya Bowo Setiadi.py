...
daily_rent, daily_casual_rent, daily_registered_rent = count_rental(main_df)
...
# Rename weathersit value to string
...
fig, ax = plt.subplots()
plt.bar(day_weathersit_total.index,day_weathersit_total['total'])
plt.xlabel('Cuaca')
plt.ylabel('Jumlah')
...
''')
st.dataframe(day_weathersit_total.head())
st.pyplot(fig)
